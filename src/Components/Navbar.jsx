...
export default function Navbar() {
  ...
  // Admin navigation with dynamic Meta campaigns
  const adminNav = useMemo(() => {
    return [
      ...
      // Meta submenu with loading state
      {
        icon: <FaUsers />,
        label: "Meta",
        ...
        submenu: loading
          ...
          : [{ icon: <FaChevronDown />, sublabel: "No Campaigns Found", path: "#" }],
      },
  ...
  }, [loading, campaignNames]);

  // User navigation dynamically from permissions
  const userNav = useMemo(
    () => permissions.map((perm) => ({ icon: <MdLeaderboard />, label: perm.label, path: perm.path })),
    [permissions]
  );
  ...
  return (
    <nav className="h-screen bg-gradient-to-b from-[#2a1ce7] to-[#1c39bb] text-white fixed top-0 left-0 shadow-xl flex flex-col border-r border-white/10 w-64 transition-all duration-300">
      ...
      <div className="px-6 py-5 border-b border-white/10">
        <h1 className="text-2xl font-bold tracking-wide">{role === "admin" ? "Admin Panel" : "User Panel"}</h1>
      </div>
            ...
            return (
              <li key={item.label}>
                ...
                >
                  ...
                  {item.submenu && <FiChevronDown className={`text-xs transition-transform ${openSubmenu === item.label ? "rotate-180" : ""}`} />}
                </div>
                        ...
                        <li key={sub.sublabel}>
                          ...
                          >
                            ...
                            <span className="truncate">
                              {sub.loading ? <span className="bg-white/30 w-28 h-3 rounded-md animate-pulse inline-block"></span> : sub.sublabel}
                            </span>
                          </div>
                        </li>
                      );
      ...
      <div className="px-4 py-4 border-t border-white/10 relative">
        ...
        {dropdownOpen && (
          <div className="absolute bottom-16 left-4 right-4 bg-white text-[#141414] rounded-lg shadow-lg overflow-hidden animate-fade-in z-50">
            <button onClick={() => handleNavigate(role === "admin" ? "/admin-profile" : "/user-profile")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">
              View Profile
            </button>
            <button onClick={handleLogOut} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 border-t">
              Logout
            </button>
          </div>
        )}
